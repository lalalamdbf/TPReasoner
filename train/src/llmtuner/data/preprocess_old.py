...
def preprocess_cpo_dataset(
    ...
    def extract_4path_index(sentence_ids, extract_ids, is_start = True):
        ...
        for i in range(len(sentence_ids) - len(extract_ids) + 1):
            if sentence_ids[i:i+len(extract_ids)] == extract_ids:
                matching_indices.append(i + len(extract_start_ids) - 1 if is_start else i - 2)
    ...
    model_inputs = {"prompt_ids": [], "response_ids": [], "extract_indices": [], "anchor_num": [], "positive_num": [], "negative_num": []}
    for i in range(len(examples["prompt"])):
        ...
        temp = [0, 1, 2, 3]
        anchor_num = [example_0_answer]
        positive_num = [example_1_answer]
        ...
        model_inputs["anchor_num"].append([anchor_num, positive_num])
        model_inputs["positive_num"].append([positive_num, anchor_num])
